import 'package:bazrin/feature/data/API/Helper/Customer/updateCustomerbyid.dart';
...
class CustomerUpdate extends StatefulWidget {
  final dynamic cus;
  const CustomerUpdate({super.key, required this.cus});
...
class _CustomerUpdateState extends State<CustomerUpdate> {
  ...
  void initState() {
    super.initState();
    usernameController.text = widget.cus['name'];
    emailController.text = widget.cus['email'];
    addressController.text = widget.cus['address'];
    phoneNumber = widget.cus['phone'];
  }
  ...
  void submitEdit() async {
    ...
    try {
      final response = await Updatecustomerbyid.UpdateCustomerByid(
        supplierUpdated,
        widget.cus['id'],
      );
      if (response == 'success') {
            ...
            child: Column(
              children: [
                Container(
                  ...
                  child: Column(
                    children: [
                      Headercomponent(title: "Update Customer"),
                      Container(
                        decoration: BoxDecoration(
                          borderRadius: BorderRadius.only(
                            bottomLeft: Radius.circular(8),
